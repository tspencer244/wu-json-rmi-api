...
public final class GetInventoryMethod extends Method<InventoryResponse> {
	...
	public InventoryResponse process(RMIClient client, Request request) throws RemoteException {
		...
		Map<String, String> inventory = client.getWebInterface().getInventory(client.getIntraServerPassword(), playerId);
		List<Item> items = new ArrayList<>();

		for (Entry<String, String> entry : inventory.entrySet()) {
			if (detailed) {
				Map<String, ?> details = client.getWebInterface().getItemSummary(client.getIntraServerPassword(), Long.parseLong(entry.getKey()));
